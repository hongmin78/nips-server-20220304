...
const parse_q_msg=str=>{
	...
	/** */
	if ( jdata && jdata?.BALLOT_ACTIVE == 'START' || ''+ jdata?.BALLOT_ACTIVE == '1' ){ // turn on
		// no-op	
	} 
	if ( jdata && jdata?.BALLOT_ACTIVE == 'PAUSE' || ''+ jdata?.BALLOT_ACTIVE == '0' ){ // turn off 
...
/********* current */
{	LOGGER(`hello world`); let current_act='BALLOT_CURRENT_ROUND_DRAW'
	findone('settings', {key_ : current_act  } ).then(resp=>{
		if(resp){}
		else {LOGGER( `${current_act} not defined` ) ; return }
		let { value_ : drawtimeunix }=resp
		if ( + drawtimeunix < moment().unix() ){ LOGGER(`@${current_act} past time`) } // past this point
		else { LOGGER(`scheduling ${current_act}`) // to reach this point
			jschedules['CURRENT_ROUND_DRAW']?.cancel ()  
			let dateobj = moment.unix ( drawtimeunix ).toDate() ; LOGGER(`@current setting ${moment.unix ( drawtimeunix ).toString()}`)
			let scheduleddrawjob = nodeschedule.scheduleJob ( dateobj , _=>{
				func00_allocate_items_to_users ( 'CURRENT' )
			})
			jschedules['CURRENT_ROUND_DRAW'] = scheduleddrawjob
...
LOGGER(`@current_act : ${current_act}`)
	findone('settings', {key_ : current_act }).then(resp=>{
		...
		else { LOGGER(`scheduling ${current_act} to ${drawtimestr }`)
			jschedules[ 'CURRENT_ROUND_PAYMENT_DUE' ]?.cancel()
			let dateobj = moment.unix ( drawtimeunix ).toDate(); LOGGER(`@current setting`)
			let scheduleddrawjob = nodeschedule.scheduleJob ( dateobj , _=>{
				func01_inspect_payments ( )
			})
			jschedules['CURRENT_ROUND_PAYMENT_DUE'] = scheduleddrawjob
...
/********* next */
	findone('settings', {key_ : 'BALLOT_NEXT_ROUND_DRAW' } ).then(resp=>{
		if(resp){}
		else {LOGGER( 'BALLOT_NEXT_ROUND_DRAW not defined' ) ; return }
		let {value_ : drawtimeunix }=resp
		if ( + drawtimeunix < moment().unix() ){ } // past this point
		else { // to reach this point
			jschedules['NEXT_ROUND']?.cancel ()  
			let dateobj = moment.unix ( drawtimeunix ).toDate() ; LOGGER(`@next setting ${moment.unix ( drawtimeunix ).toString()}`)
			let scheduleddrawjob = nodeschedule.scheduleJob ( dateobj , _=>{
				func00_allocate_items_to_users ( 'NEXT' )
			})
	...
	findone('settings', {key_ : 'BALLOT_NEXT_ROUND_PAYMENT_DUE' }).then(resp=>{
		if(resp){}
		else { LOGGER( 'BALLOT_NEXT_ROUND_PAYMENT_DUE not defined'); return }
		let { value_ : drawtimeunix } = resp
		if ( +drawtimeunix < moment().unix() ){} // 
		else {
			jschedules['NEXT_ROUND_PAYMENT_DUE']?.cancel()
