...
const pick_kong_items_on_item_max_round_reached = async (MAX_ROUND_REACH_RELATED_PARAMS, nettype) => {
...
const handle_perish_item_case = async (itemid, nettype) => {
  ...
  await moverow("circulations", { id: respcirculation.id }, "logcirculations", { txhash });
  await incrementrow({
    table: "users",
...
const handle_give_an_item_ownership_case = async (username, nettype) => {
  let respitem = await getrandomrow_filter("items", { nettype, group_: "kingkong", salestatus: 0 });
  if (respitem) {
  } else {
    createrow("alerts", {
