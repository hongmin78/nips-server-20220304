...
const { findone, createrow, updaterow, countrows_scalar } = require("../utils/db");
...
let transporter = nodemailer.createTransport({
  service: "gmail",
  auth: { user: "testnodemailer77@gmail.com", pass: "chainlabs123" },
});
    ...
    (error, info) => {
      if (error) {
        ...
        findone(TABLENAME, { emailaddress: email }).then((resp) => {
          ...
          respok(res, { code: authNum }); //        res.status(200).send("이메일 전송 성공");
        });
      }
    }
  );
...
router.post("/signup", async (req, res) => {
  ...
  if (respreferer) {
    ...
    resperr(res, messages.MSG_INVALID_REFERER);
    return;
  }
  let count_users = await countrows_scalar("users", { email });
  if (count_users > 11) {
    resperr(res, "ERR_EMAIL_COUNTING", null, { reason: "emailcounting" });
    return;
  } else {
  }
  ...
  if (respaddress) {
    resperr(res, messages.MSG_DATADUPLICATE, null, { reason: "walletaddress" });
    return;
  } else {
  }
  findone("users", { address: walletaddress }).then(async (resp) => {
    // M_userInfo.find({ walletAddress }, (err, item) => {
    if (resp) {
      resperr(res, messages.MSG_DATADUPLICATE, null, { reason: "walletaddress" });
      return;
    } else {
    } //    if (item[0]) res.status(400).send("이미 가입된 지갑입니다");
...
router.post("/email/auth", async (req, res) => {
  LOGGER(req.body);
  const { email: emailaddress, authNum: code, walletaddress } = req.body; //  M_emailAuth.updateOne(
