...
const handle_perish_item_case = async (itemid, nettype, username) => {
  ...
  await deleterow("receivables", { itemid, nettype });
  return true;
}; //
const get_price_of_kingkong_upon_birth = async (nettype) => {
  const PRICE_OF_KINGKONG_UPON_BIRTH_DEF = "372";
  let { value_ } = await findone("settings", {
    key_: "PRICE_OF_KINGKONG_UPON_BIRTH",
    nettype,
  });
  if (value_) {
  } else {
    return PRICE_OF_KINGKONG_UPON_BIRTH_DEF;
  }
  return value_;
};
const handle_give_an_item_ownership_case_this_ver_charges_for_payment = async (
  username,
  nettype
) => {
  ...
  if (respitem) {
    ...
    LOGGER(`@give kingkong, return on empty reserve`);
    return null;
  }
  let { itemid } = respitem;
...
const handle_give_an_item_ownership_case_this_ver_gives_for_free = async (
  username,
  nettype
) => {
  let respitem = await getrandomrow_filter("items", {
...
const handle_give_an_item_ownership_case =
  handle_give_an_item_ownership_case_this_ver_charges_for_payment;
...
const get_price_of_kong_upon_birth = async (nettype) => {
  const PRICE_OF_KONG_UPON_BIRTH_DEF = "157.35";
  let { value_ } = await findone("settings", {
    key_: "PRICE_OF_KONG_UPON_BIRTH",
    nettype,
  });
  if (value_) {
  } else {
    return PRICE_OF_KONG_UPON_BIRTH_DEF;
  }
  return value_;
};
const get_roundnumber_of_kong_on_birth = async (nettype) => {
  const ROUNDNUMBER_OF_KONG_ON_BIRTH_DEF = 3;
  let { value_ } = await findone("settings", {
    key_: "ROUNDNUMBER_OF_KONG_ON_BIRTH",
    nettype,
  });
  if (value_) {
  } else {
    return ROUNDNUMBER_OF_KONG_ON_BIRTH_DEF;
  }
  return value_;
};
